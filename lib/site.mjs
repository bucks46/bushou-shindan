...
// 【2026-08-14 CEO指摘で追加】afbのサブID機能(&id1=xxx)を使い、武将別にクリック実績を
// 分離計測できるようにする。全武将が同一URLを共有していたため、CTA一本化(8/7)後の
// 効果検証が「全体の合算」でしか見られず、note/診断結果いずれの導線が効いているか
// 切り分けられない状態だった。afb管理画面の「キーワード」欄にid1の値がそのまま表示される
// （https://www.afi-b.com/guide/how-to-connect-link-codes/）。
export function agarootCtaForWarrior(warriorId) {
  const match = agarootMatches[warriorId];
  const base = CTA_CONTENT.agaroot;
  if (!match) return { ...base, url: `${base.url}&id1=w${warriorId}` }; // 未マッチ武将もサブID付与
  return {
    ...base,
    url: `${base.url}&id1=w${warriorId}`,
